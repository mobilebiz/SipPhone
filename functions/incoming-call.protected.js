...
exports.handler = function(context, event, callback) {
  ...
  // 電話帳アプリからデータを取得
  addressClient.record.getRecords({
    app: addressAppId,
    fields: ['number', 'deny'],
    query: `number = "${from}"`,
    totalCount: true,
  ...
  .then(resp => {
    if (resp.totalCount === '0') {
      ...
      if (resp.records[0].deny.value.filter(v => v === '拒否する').length === 1) fDeny = true;  // 着信拒否設定がされている
      return true;
    }
  })
  .then(() => {
...
const addAddress = (from) => {
  return new Promise((resolve, reject) => {
    ...
    addressClient.record.addRecord({
      app: addressAppId,
      record: record
    })
    .then(() => {
...
const addHistory = (from) => {
  return new Promise((resolve, reject) => {
    ...
    historyClient.record.addRecord({
      app: historyAppId,
      record: record
    })
    .then(() => {
